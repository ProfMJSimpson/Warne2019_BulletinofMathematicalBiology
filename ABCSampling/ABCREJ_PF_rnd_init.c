/* approximate Bayesian Computation  for inference on the Porous Fisher Model
 * with randomised initial conditions
...
/* ABC rejection for inference of Porous-Fisher parameters 
...
simulate(void *sim, double *theta, Dataset *data_s)
{
    ...
    C_std = (double *)data.fields[4].data_array;
    C_s = (double *)data_s->fields[3].data_array;
    ...
    for (int i=1;i<nx;i++)
    {
        ...
        double C_l,C_u;
        C_l = durngns(C0[i-1],4.0*C_std[i-1]); /*adding noise*/
        C_u = durngns(C0[i],4.0*C_std[i]);
        C_l = (C_l >= 0.0) ? C_l : 0.0;
        C_u = (C_u >= 0.0) ? C_u : 0.0;
        for (int j=0;j<=R+1;j++)
        {
...
ImportCellCountData(char * filename, Dataset* data)
{
    ...
    c_std = (double*)data->fields[4].data_array;
    for (int j=0;j<data->fields[4].numRows;j++)
    {
        for (int i=0;i<data->fields[4].numCols;i++)
        {
            c_std[j*(data->fields[4].numCols) + i] = buffer[(j+1)*(numCols) + i + 2 + numCols/2];
        }
    }
    free(buffer);
...
main(int argc, char** argv)
{
    ...
    ImportCellCountData(filename,&data);
    {
        ...
        fprintf(stderr,"T=00.000000 : %f %f ... %f %f %f...%f %f\n", 
                 ...
                 c0[nx -1]);
        fprintf(stderr,"std : %f %f ... %f %f %f...%f %f\n",
               c_std[0],c_std[1],c_std[nx/2 -1], c_std[nx/2],
               c_std[nx/2 +1],c_std[nx -2],c_std[nx -1]);
        for (int i=0;i<nt;i++)
        {
            ...
            fprintf(stderr,"std : %f %f ... %f %f %f...%f %f\n",
                   c_std[i*nx],c_std[i*nx + 1],c_std[i*nx + nx/2 -1], c_std[i*nx + nx/2],
                   c_std[i*nx + nx/2 +1],c_std[i*nx + nx -2],c_std[i*nx +nx -1]);
        }
    }
    ...
    fprintf(stdout,"\"Sample\",\"D\",\"lambda\",\"K\",\"rho\",\"SEC\",\"NACC\",\"NSIMS\",\"NFAIL\"\n");
    for (int i=0;i<abc_p.nacc;i++)
    {
        fprintf(stdout,"%d,%g,%g,%g,%g,%g,%d,%d,%d\n",i,theta[i*abc_p.k],theta[i*abc_p.k+1],theta[i*abc_p.k+2],d[i],time,abc_p.nacc,sim_counter,failure_counter);
        fflush(stdout);
    }
